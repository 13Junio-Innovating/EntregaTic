...
if (!$sale_values_decoder) {
    redirect(["error" => "decode", "message" => "Erro ao decodificar JSON", "debug" => $_POST]);
}
...
if (!isset($csrf_token) || $csrf_token !== $_SESSION['csrf_token']) {
    ...
    if ($action == "register_sale") {
        ...
        $signature = $sale_values_decoder->{"signature-data"} ?? '';
        $assinatura_path = '';
        ...
        if (!empty($signature)) {
            ...
            //$assinatura_dir = __DIR__ . "/var/www/html/entregatic/app/assets/img/assinaturas/";
            $assinatura_dir = "/var/www/html/entregatic/app/assets/img/assinaturas/";


            if (!file_exists($assinatura_dir)) {
                mkdir($assinatura_dir, 0777, true);
            }

            $signature_filename = "assinatura_" . time() . "_" . rand(1000,9999) . ".png";
            file_put_contents($assinatura_dir . $signature_filename, $fileData);

             $assinatura_path = 'assets/img/assinaturas/' . $signature_filename;
        ...
        foreach ($sale_values_decoder->products as $product) {
            ...
            $model_sale->__set("assinatura", $signature_filename);
            $model_sale->__set("patrimonio", isset($product->patrimony) ? $product->patrimony : null); 
            $model_sale->__set("itsm_ticket", isset($product->itsm_ticket) ? $product->itsm_ticket : null);
            $model_sale->__set("hostname", isset($product->hostname) ? $product->hostname : null);
        ...
        if (in_array(0, $return)) {
            ...
            redirect(array("success" => "sucesso", "id" => strval($count), "message" => "Entrega de Termo realizado com sucesso." . $count));
        }
    } else {
