...
if (!isset($csrf_token) || $csrf_token != $_SESSION['csrf_token']) {
    ...
    if (isset($action) && $action === "add_client") {
        ...
        if ($result) {
            ...
            $celula = $new_client_values_decode->celula;
            $predio = $new_client_values_decode->predio;
            $cargo = $new_client_values_decode->cargo;
            $empresa = $new_client_values_decode->empresa;
            ...
            $model_client->__set("celula", $celula);
            $model_client->__set("predio", $predio);
            $model_client->__set("cargo", $cargo);
            $model_client->__set("empresa", $empresa);
