...
if ($service_user->recoverPassword()) {
    ...
    if ($saveTokenResetPassword) {
        ...
        $mensagem = "<html>
                        ...
                        <body>
                            <div class=\"container\">
                                <h2>Recuperação de senha - Entrega TIC</h2>
                                <p>Você solicitou a recuperação de senha do Entrega TIC.
        ...
        $mensagem .= "
                                ...
                                <p><a  href=\"http://entregatic.costao.com.br/web/alterar-senha?u=$user_crypt&t=$token_crypt\">Alterar senha</a></p>
                                <p>O link é válido por 10 minutos</p>
                                <p>Atenciosamente,</p>
                                <p>Equipe TIC Costão</p>     
        ...
        $mail->Host       = 'mail.costao.com.br';  // SEU HOST (HOSPEDAGEM) - nesse caso deixei para o gmail
        $mail->SMTPAuth   = false;                        // Manter em true
        $mail->Username   = 'suporte@costao.com.br';
        ...
        //Recipients
        $mail->setFrom('suporte@costao.com.br', 'Entrega TIC');  //DEVE SER O MESMO EMAIL DO USERNAME
        $mail->addAddress($email_recover);     // QUAL EMAIL RECEBERÁ A MENSAGEM!
        //$mail->addBCC(''); //ADICIONANDO BCC

        // Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = 'Recuperação de senha Entrega TIC'; //ASSUNTO
