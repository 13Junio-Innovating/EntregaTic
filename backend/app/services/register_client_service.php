...
class RegisterClientService
{
    ...
    {
        $query = "INSERT INTO tb_clientes (nome, cpf, rg, data_nascimento, idade, cep, cidade, uf, endereco, bairro, complemento, numero, ponto_referencia, telefone, celular, email, celula, predio, cargo, empresa) VALUES (:nome, :cpf, :rg, :data_nascimento, :idade, :cep, :cidade, :uf, :endereco, :bairro, :complemento, :numero, :ponto_referencia, :telefone, :celular, :email, :celula, :predio, :cargo, :empresa)";
        $stmt = $this->connect->prepare($query);
        ...
        $stmt->bindValue(':celula', $this->client->__get('celula'));
        $stmt->bindValue(':predio', $this->client->__get('predio'));
        $stmt->bindValue(':cargo', $this->client->__get('cargo'));
        $stmt->bindValue(':empresa', $this->client->__get('empresa'));
        $stmt->execute();
    ...
    {
        $query = "UPDATE tb_clientes SET nome = :nome, cpf = :cpf, rg = :rg, data_nascimento = :data_nascimento, idade = :idade, cep = :cep, cidade = :cidade, uf = :uf, endereco = :endereco, bairro = :bairro, complemento = :complemento, numero = :numero, ponto_referencia = :ponto_referencia, telefone = :telefone, celular = :celular, email = :email, celula = :celula, predio = :predio, cargo = :cargo, empresa = :empresa WHERE id = :id";
        $stmt = $this->connect->prepare($query);
        $stmt->bindValue(':nome', $this->client->__get('nome'));
        $stmt->bindValue(':cpf', $this->client->__get('cpf'));
        $stmt->bindValue(':rg', $this->client->__get('rg'));
