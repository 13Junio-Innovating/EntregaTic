...
if (!isset($_GET['action']) || !isset($_GET['id'])) {
    ...
    $action = $_GET['action'];
    if ($action === "print_report_qrcode") {
        $sale_id = $_GET['id'];

        $query = "SELECT v.*, c.nome, c.cpf, p.nome_produto, p.codigo_produto, p.id_doc_controletic FROM tb_vendas v JOIN tb_clientes c ON v.cliente = c.id JOIN tb_produtos p ON v.produto = p.id WHERE v.numero_da_venda = :sale_id";
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output("Termo_Entrega_" . $sale_id . ".pdf", "I");
        exit();
