...
function clearInputs() {
    ...
    if (url_page === "cadastro-colaborador") {
        let inputs = [
            ...
            'celula', 'predio', 'cargo', 'empresa', "search-client"
        ]
    ...
    if (url_page === "cadastro-produto") {
        let inputs = [
            ...
            'product-unit-price', 'product-sale-price', 'total-price-on-product', 'img-product', 'file-name', 
            'product-type', 'product-so', 'product-cpu', 'product-memory', 'search-product'
        ]
        for (let i = 0; i < inputs.length; i++) {
            document.querySelector(`#${inputs[i]}`).value = "";
        }
...
//validação dos campos de inputs da página de cadastro de colaborador
...
// mascaras dos inputs da página de cadastro de colaborador e fornecedor
    ...
    formAddNewClient.addEventListener("submit", function (e) {
        ...
        if (validateInputs()) {
            ...
            newClientValues = {
                ...
                celula: document.querySelector('#celula').value,
                predio: document.querySelector('#predio').value,
            ...
            $.ajax({
                ...
                error: function (response) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        text: 'Erro ao cadastrar Colaborador'
                    });
                }
            });
        }
        ...
        if (valueSearch != "") {
            ...
            $.ajax({
                    ...
                    if (response.error) {
                        ...
                        for (let i = 0; i < response.length; i++) {
                            ...
                            let clientObj = {
                                ...
                                celula: response[i].celula,
                                predio: response[i].predio,
                            ...
                            icon_edit.addEventListener("click", function () {
                                ...
                                document.querySelector('#celula').value = clientObj.celula;
                                document.querySelector('#predio').value = clientObj.predio;
                                document.querySelector('#cargo').value = clientObj.cargo;
                                    ...
                                    if (validateInputs()) {
                                        ...
                                        $.ajax({
                                            ...
                                            error: function (response) {
                                                ...
                                                Swal.fire({
                                                    icon: 'error',
                                                    text: 'Erro ao atualizar Colaborador',
                                                    allowOutsideClick: false
                                                });
                                            }
                                        });
                ...
                error: function (response) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        text: 'Erro ao buscar Colaborador',
                        allowOutsideClick: false
                    });
                }
            });
...
function deleteClient(id) {
    Swal.fire({
        icon: 'warning',
        text: 'Deseja realmente excluir esse Colaborador?',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                error: function (response) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        text: 'Erro ao excluir colaborador',
                        allowOutsideClick: false
                    });
                }
            });
...
// Habilitar/desabilitar campos conforme tipo de produto
document.addEventListener("DOMContentLoaded", function () {
    const tipoSelect = document.getElementById("product-type");
    const campoImei = document.getElementById("product-imei");
    const campoEmail = document.getElementById("product-email");
    const camposGerais = [
        document.getElementById("product-storage"),
        document.getElementById("product-so"),
        document.getElementById("product-cpu"),
        document.getElementById("product-memory")
    ];
    ...
    function atualizarCamposProduto() {
        ...
        if (tipo === "Tablet") {
        ...
        } else if (["Notebook", "Desktop", "MiniPC", "Firewall", "Switch"].includes(tipo)) {
            // Habilita campos gerais, desabilita IMEI
            campoImei.disabled = true;
            campoImei.required = false;
            campoEmail.disabled = true;
            campoEmail.required = false;
            ...
            // Habilita campo Email e desabilita os demais
    ...
    if (tipoSelect) {
        tipoSelect.addEventListener("change", atualizarCamposProduto);
        atualizarCamposProduto(); // Executa no carregamento da página
    }
});
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.error) {
                        ...
                        th = document.createElement("th");
                        th.setAttribute("scope", "col");
                        th.innerHTML = "ID Documentos Controle TIC";
                        tr.appendChild(th);
                        ...
                        for (let i = 0; i < response.length; i++) {
                            ...
                            let productObj = {
                                ...
                                product_id_controletic: response[i].id_doc_controletic,
                                product_type: response[i].tipo_produto,
                                product_so: response[i].so_produto,
                                product_memory: response[i].memoria_produto,
                                product_cpu: response[i].processador_produto,
                                product_imei: response[i].imei_produto,
                                product_storage: response[i].storage_produto,
                                product_email: response[i].email_produto,
                            ...
                            td = document.createElement('td')
                            td.innerHTML = productObj.product_id_controletic;
                            tr.appendChild(td);
                            ...
                            icon_edit.addEventListener("click", function () {
                                ...
                                document.querySelector("#id-controle-tic").value = productObj.product_id_controletic;
                                document.querySelector("#product-type").value = productObj.product_type;
                                document.querySelector("#product-so").value = productObj.product_so;
                                document.querySelector("#product-memory").value = productObj.product_memory;
                                document.querySelector("#product-cpu").value = productObj.product_cpu;
                                document.querySelector("#product-imei").value = productObj.product_imei;
                                document.querySelector("#product-storage").value = productObj.product_storage;
                                document.querySelector("#product-email").value = productObj.product_email;
...
function gerarCodigo() {
    const codigo = 'TIC-' + Math.floor(100000 + Math.random() * 900000);
